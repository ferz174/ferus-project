	...
	<script charset='utf-8' src='{normalize-space(@Ресурс)}/lib/jquery-latest.js'></script>
		...
		<body>
			...
			<script type='text/javascript'>
				...
				var FileOut = PathModel+"/"+XS_INF_BOLTSNOTES+"/Shop.txt";
			...
			<script type='text/javascript'>
				<![CDATA[
					...
					for (var i = 2; i < StrDataTable.length; i++)
					{
						var NoteId = outidx('Примечание');
						var Note = StrDataTable[i].getElementsByTagName("td")[NoteId].innerText;
						if (!Note.indexOf("Ошибка длины !") || !Note.indexOf("> 3 шайб !!!")) StrDataTable[i].bgColor = "fdbcb4";
					}
