        ...
        public SwapHandles()
        {
            ...
            while (myEnum.MoveNext())
            {
				...
				else if(myEnum.Current is Tekla.Structures.Model.ContourPlate)
                {
                    ...
                    ArrayList arryPoints =  cPlate.Contour.ContourPoints;

                    cPlate.Contour.ContourPoints = arryPoints;
                     
                    for (int i = 0; i < arryPoints.Count; i++ )
                    {
                        if(i  < arryPoints.Count - 1)
                        {
                            object p1 = arryPoints[i];
                            arryPoints[i] = arryPoints[i + 1];
                            arryPoints[i + 1] = p1;
                        }						
                    }    
                    cPlate.Modify();                  
                ...
                else if(myEnum.Current is Tekla.Structures.Model.BooleanPart)
                {
                    ...
                    {                       
                        ArrayList arryPoints = boolConturPlate.Contour.ContourPoints;
        
                        boolConturPlate.Contour.ContourPoints = arryPoints;

                        for (int i = 0; i < arryPoints.Count; i++)
                        {
                            if (i > 0)
                            {
                                object p1 = arryPoints[i - 1];
                                arryPoints[i - 1] = arryPoints[i];
                                arryPoints[i] = p1;
                            }
                        }
